-- 插入默认的AI提供商（不包含API Key，需要用户自行配置）
...
SELECT id, 'gemini-2.5-pro', 'Google Gemini 2.5 Pro', 'google/gemini-2.5-pro', 'Google最新的Gemini 2.5 Pro模型', 2097152, 0.01, 1 FROM ai_providers WHERE name = 'openrouter';

INSERT OR IGNORE INTO ai_models (provider_id, name, display_name, model_id, description, max_tokens, temperature, is_default)
SELECT id, 'kimi-k2-free', 'Kimi K2 (免费)', 'moonshotai/kimi-k2:free', '月之暗面Kimi K2免费版本', 32000, 0.01, 0 FROM ai_providers WHERE name = 'openrouter';
